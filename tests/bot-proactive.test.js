...
test("cost 70% triggers info alert", async () => {
  const alerts = await runChecks(mkHelpers({ cost: { pct_used: "70.0%", spent_usd: "35.00", projected_month_usd: "50.00" } }));
...
test("cost 90% triggers warning", async () => {
  const alerts = await runChecks(mkHelpers({ cost: { pct_used: "90.0%", spent_usd: "45.00", projected_month_usd: "55.00" } }));
...
test("cost 96% triggers critical", async () => {
  const alerts = await runChecks(mkHelpers({ cost: { pct_used: "96.0%", spent_usd: "48.00", projected_month_usd: "60.00" } }));
